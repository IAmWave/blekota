...
class GRULayer:
    ...
    def forward(self, x, initial_h=None):
        ...
        for t in range(self.t_n):
            ...
            z[t] = sigmoid(np.dot(self.Wz, np.r_[x[t], h_prev, np.ones((1, b))]))
            r[t] = sigmoid(np.dot(self.Wr, np.r_[x[t], h_prev, np.ones((1, b))]))
            q[t] = r[t] * h_prev
            h0[t] = np.tanh(np.dot(self.W, np.r_[x[t], q[t], np.ones((1, b))]))
    ...
    def backward(self, dh):
        ...
        for t in reversed(range(self.t_n)):
            ...
            dh0[t] = dh[t] * z[t]
            dh0i = dh0[t] * (1 - h0[t] ** 2)
            dW += np.dot(dh0i, np.r_[x[t], q[t], np.ones((1, b))].T)
            dxq = np.dot(W.T, dh0i)
            dx[t] += dxq[:self.x_n]
            dq = dxq[self.x_n:-1]
            # r
            dr = dq * h_prev
            dri = r[t] * (1 - r[t]) * dr
            dWr += np.dot(dri, np.r_[x[t], h_prev, np.ones((1, b))].T)
            dxh = np.dot(Wr.T, dri)
            # z
            dz[t] = dh[t] * (h0[t] - h_prev)
            dzi = z[t] * (1 - z[t]) * dz[t]
            dWz += np.dot(dzi, np.r_[x[t], h_prev, np.ones((1, b))].T)
            dxh += np.dot(Wz.T, dzi)
            dx[t] += dxh[:self.x_n]
            if t > 0:
